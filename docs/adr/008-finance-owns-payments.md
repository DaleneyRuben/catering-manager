...
A Payment stores its own `amount` (`DECIMAL(10,2)`, matching `Plan.price`), its own `paidAt`,
its own `clientId`, and a **nullable** `subscriptionId` with `ON DELETE SET NULL`. It exposes
one write function, `recordPayment`. There is no update and no delete.
...
- **`markPaid` has no undo, and now it moves money.** Nothing in the codebase reverses
  ...
  income row that cannot be removed. Reversing it would mean un-deferring history, un-finalizing
...
- **Totals are summed in SQL, never in JavaScript.** `pg` returns `DECIMAL` as a string (which is
  why `ActivePlanCard.tsx` already does `Number(sub.plan.price)`). Aggregations use `SUM(amount)`
