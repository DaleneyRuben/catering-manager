...
**Prerequisite: the plan-change control must ship first** ([`plan-change-implementation.md`](./plan-change-implementation.md)). Until it does, admins simulate plan changes by finalizing and recreating a subscription, which produces two paid subscriptions for a client who paid once — and this register would count both.
...
- **`payments`** — `clientId` FK **not null**, `subscriptionId` FK **nullable with `ON DELETE SET NULL`**, `amount` `DECIMAL(10,2)`, `paidAt` `DATEONLY`, `registeredBy` FK to `users`, timestamps. Not paranoid, and never updated or deleted.
...
- `record-payment.ts` → `recordPayment(data, actor, transaction?)`. The **only** way a payment is ever written. No update, no delete — don't add them "for symmetry".
...
Amount is the subscription's total (`plan.price − discount`) **resolved at that moment** and stored on the row. Never recompute income from a join to `plans` later: a plan change would silently rewrite past months. `paidAt` is the day it happens. `registeredBy` is the acting `Actor`.
