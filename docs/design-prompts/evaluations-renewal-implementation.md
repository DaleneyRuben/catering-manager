...
> Companion to [`evaluations.md`](./evaluations.md) (the original design prompt) and [`evaluations-existing-client-renewal.md`](./evaluations-existing-client-renewal.md) (the follow-up design prompt covering this feature's new UI). Those two cover visual/layout specifics; this one is a Claude Code work breakdown for actually implementing the behavior.
