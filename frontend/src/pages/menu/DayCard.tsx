...
export function DayCard({ isToday, date, menu, isWeekend, onOpen }: Props) {
  ...
  return (
    <div
      className={
        isToday
          ? 'bg-gradient-to-br from-[#21420c] to-[#163008] rounded-[14px] px-[26px] py-6 flex items-center justify-between gap-5 shadow-[0_8px_24px_rgba(20,40,6,0.14)]'
    ...
    >
      <div>
          ...
          <span
            className={
              isToday
                ...
                : 'w-[7px] h-[7px] rounded-full bg-[#cdd0bf] shrink-0'
            }
          />
          <span
        ...
        <p className={`text-[13px] mt-[7px] ${isToday ? 'text-[#bcd596]' : 'text-faint'}`}>
          {statusText}
        </p>
      </div>
      ...
      {isWeekend ? (
        <div
          className={
            isToday
              ...
              : 'inline-flex items-center gap-2 bg-[#f3f2ec] text-faint border border-[#e4e1d3] rounded-[9px] px-[18px] py-3 text-[13px] font-semibold whitespace-nowrap shrink-0 cursor-not-allowed'
          }
        >
        ...
        <button
          ...
          className={
            isToday
              ? 'inline-flex items-center gap-2 bg-[#f3f6ec] text-[#1e3c0a] rounded-[9px] px-5 py-3 text-[13.5px] font-bold whitespace-nowrap shrink-0 hover:bg-white transition-colors'
