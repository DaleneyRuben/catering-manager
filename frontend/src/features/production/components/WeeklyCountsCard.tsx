...
export function WeeklyCountsCard({ weeklyCounts, today, isAdmin = false }: Props) {
  const [offset, setOffset] = useState(0);
  const [selectedDate, setSelectedDate] = useState<string | null>(null);
  const { weeklyCounts: offsetCounts, isLoading } = useWeeklyCounts(offset);

  const shown = offset === 0 ? weeklyCounts : offsetCounts;

  const renderCells = () => {
    if (!shown) return <WeekCellsSkeleton />;
    return (
      <div className="grid grid-cols-5 gap-3.5 max-md:grid-cols-1">
        {shown.days.map((day) => {
  ...
  return (
    <div className="bg-paper border border-rule rounded-[14px] px-7 py-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {isAdmin && (
            <IconButton
              ...
              onClick={() => setOffset((o) => Math.min(MAX_WEEK_OFFSET, o + 1))}
              className={NAV_BUTTON_CLASS}
            />
          )}
        </div>
      ...
      {isLoading ? <WeekCellsSkeleton /> : renderCells()}
